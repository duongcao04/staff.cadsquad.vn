...
@CommandHandler(SoftDeleteJobCommand)
export class SoftDeleteJobHandler implements ICommandHandler<SoftDeleteJobCommand> {
	constructor(
	...
	async execute(command: SoftDeleteJobCommand) {
		...
		if (job.deletedAt) throw new BadRequestException('Job is already deleted');
