...
export const Route = createFileRoute(
  ...
  component: PayoutJobDetail,
})

function RouteComponent() {
  return <div>Hello "/_administrator/financial/pending-payouts/$no"!</div>
}
...
const getMockJobDetail = (no: string) => ({
    ...
    assignees: [
        { id: '1', name: 'Cao Hai Duong', role: '3D Artist', cost: 100, avatar: 'https://i.pravatar.cc/150?u=1' },
        { id: '2', name: 'John Doe', role: 'Reviewer', cost: 50, avatar: 'https://i.pravatar.cc/150?u=2' },
    ],
    totalCost: 150,
})
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            <div>
                ...
                <p className="text-sm text-default-500">Job #{job.no} • {job.displayName}</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-7 space-y-6">
                    <Card shadow="sm" className="border border-divider">
                        ...
                        <CardBody className="p-6 space-y-6">
                            {/* Income Block */}
                            <div>
                                <p className="text-xs font-bold text-default-400 uppercase tracking-wider mb-3">Client Payment (In)</p>
                                <div className="flex items-center justify-between p-4 bg-success-50 rounded-xl border border-success-100">
                                    <div>
                                        <p className="text-sm font-semibold text-success-900">{job.clientName}</p>
                                        <p className="text-xs text-success-700">via {job.paymentChannel} • {job.completedAt}</p>
                                    </div>
                                    <p className="text-xl font-bold text-success-700">{formatCurrency(job.income)}</p>
                                </div>
                            </div>

                            {/* Staff Cost Block */}
                            <div>
                                <p className="text-xs font-bold text-default-400 uppercase tracking-wider mb-3">Staff Distribution (Out)</p>
                                <div className="space-y-3">
                                    {job.assignees.map(user => (
                                        <div key={user.id} className="flex items-center justify-between p-3 border border-default-200 rounded-lg">
                                            <User 
                                                name={<span className="font-semibold text-sm">{user.name}</span>}
                                                description={user.role}
                                                avatarProps={{ src: user.avatar, size: 'sm' }}
                                            />
                                            <p className="font-bold text-danger-600">{formatCurrency(user.cost)}</p>
                                        </div>
                                    ))}
                                </div>
                                <div className="flex justify-between items-center mt-3 pt-3 border-t border-divider">
                                    <span className="font-semibold text-sm text-default-600">Total Payout Required:</span>
                                    <span className="font-bold text-lg text-danger-600">{formatCurrency(job.totalCost)}</span>
                                </div>
                            </div>

                            {/* Margin Block */}
                            <div className="flex justify-between items-center p-4 bg-primary-50 rounded-xl border border-primary-100">
                                <span className="font-bold text-sm text-primary-900">Net Company Profit:</span>
                                <span className="font-bold text-xl text-primary-700">{formatCurrency(profit)}</span>
                            </div>
                        </CardBody>
                    </Card>
                </div>
                ...
                <div className="lg:col-span-5">
                    <Card shadow="md" className="border border-default-200 sticky top-6">
                        <form onSubmit={formik.handleSubmit}>
                            <CardHeader className="px-6 py-5 border-b border-divider">
                                <h2 className="text-lg font-bold flex items-center gap-2">
                                    <BadgeCheck className="text-success" /> Confirm Transfer
                                </h2>
                            </CardHeader>
                            <CardBody className="p-6 space-y-5">
                                ...
                                <Select 
                                    ...
                                    onChange={formik.handleChange}
                                >
                                    <SelectItem key="vpbank" textValue="VP Bank">VP Bank Transfer</SelectItem>
                                    <SelectItem key="paypal" textValue="PayPal">PayPal</SelectItem>
                                    <SelectItem key="wise" textValue="Wise">Wise</SelectItem>
                                </Select>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <label className="cursor-pointer border-2 border-dashed border-default-300 hover:border-primary hover:bg-primary-50 transition-all rounded-xl p-4 flex flex-col items-center justify-center gap-2">
                                        ...
                                        <UploadCloud size={24} className="text-default-400" />
                                        <span className="text-sm text-default-500">Upload screenshot (JPG, PDF)</span>
                                    </label>
                                </div>
